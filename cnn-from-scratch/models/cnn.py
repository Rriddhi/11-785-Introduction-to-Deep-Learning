...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(self.nlayers):
            in_channels = num_input_channels if i == 0 else num_channels[i - 1]
            out_channels = num_channels[i]
            kernel_size = kernel_sizes[i]
            stride = strides[i]
            output_width = (output_width - kernel_sizes[i]) // strides[i] + 1
            conv_layer = Conv1d(in_channels, out_channels, kernel_size, stride, padding=0, weight_init_fn=conv_weight_init_fn, bias_init_fn=bias_init_fn)
            self.convolutional_layers.append(conv_layer)
        self.flatten = Flatten()
        self.linear_layer = Linear(out_channels * output_width, num_linear_neurons)
        self.layers = []
        for i in range(self.nlayers):

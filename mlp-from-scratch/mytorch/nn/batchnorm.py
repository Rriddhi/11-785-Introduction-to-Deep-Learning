...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        dNZdM = (-(self.V + self.eps) ** (-0.5)- 0.5 * (self.Z - self.M) * (self.V + self.eps) ** (-1.5)* (-2.0 / self.N) * np.sum(self.Z - self.M,axis=0, keepdims=True)) # TODO: Compute derivative of normalized input with respect to mean.
        dLdM = np.sum(dLdNZ*dNZdM,axis=0, keepdims=True)  # TODO: Compute gradient of loss with respect to mean.

        dLdZ = dLdNZ*((self.V+self.eps)**(-0.5)) + dLdV*((2/self.N)*(self.Z-self.M)) +(1/self.N)*dLdM  # TODO: Compute gradient of loss with respect to the input.

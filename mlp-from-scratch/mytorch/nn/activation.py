...
class GELU:
    ...
    def forward(self, Z): 
        self.Z = Z
        self.A = 0.5*Z*(1+ np.vectorize(math.erf)(Z / np.sqrt(2)))
        return self.A
    
    def backward(self, dLdA):
        dLdZ = dLdA * (0.5 * (1 + np.vectorize(math.erf)(self.Z / np.sqrt(2)))+ (self.Z / np.sqrt(2 * math.pi)) * np.exp(-0.5 * self.Z * self.Z))
        return dLdZ
...
class Swish:
    ...
    def backward(self,dLdA):
        dAdZ = (1 / (1 + np.exp(-self.beta*self.Z))) + self.beta * self.Z * (1 / (1 + np.exp(-self.beta*self.Z))) *(1-(1 / (1 + np.exp(-self.beta*self.Z))) )
        dLdZ = dLdA * dAdZ
        dAdbeta = self.Z*self.Z*(1 / (1 + np.exp(-self.beta*self.Z)))*(1-(1 / (1 + np.exp(-self.beta*self.Z))))
        self.dLdbeta = np.sum(dAdbeta*dLdA)
        return dLdZ
...
class Softmax:
    ...
    def forward(self, Z):
        ...
        self.A = np.exp(Z_stable)/np.sum(np.exp(Z_stable), axis = 1, keepdims=True)# TODO
